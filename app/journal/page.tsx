                    ...
                    <div className="flex flex-col gap-1 w-full sm:w-auto">
                        ...
                        <div className="relative">
                            <select
                                ...
                                className="w-full sm:w-48 appearance-none bg-white/10 hover:bg-white/15 border border-white/10 rounded-xl px-4 py-3 pr-10 outline-none focus:ring-2 focus:ring-blue-500/50 transition-colors cursor-pointer"
                            >
                                {MOCK_GROUPS.map((g) => (
                                    <option
                                        key={g.id}
                                        value={g.name}
                                        className="bg-white-900 " //выпадающий список групп
                                    >
                                        {g.name}
                                    </option>
                                ))}
                    ...
                    <div className="flex flex-col gap-1 w-full sm:w-auto">
                        ...
                        <div className="relative">
                            <select
                                ...
                                className="w-full sm:w-64 appearance-none bg-white/10 hover:bg-white/15 border border-white/10 rounded-xl  px-4 py-3 pr-10 outline-none focus:ring-2 focus:ring-blue-500/50 transition-colors cursor-pointer"
                            >
                                {ALL_SUBJECTS.map((s) => (
                                    <option
                                        key={s}
                ...
                <GlassCard className="!rounded-full" intensity="low">
                    <div className="p-1 rounded-full flex relative w-full md:w-auto">
                        ...
                        <button
                            ...
                            className={`flex-1 md:w-36 py-2 rounded-full relative z-10 text-sm font-semibold transition-colors duration-300 flex items-center justify-center gap-2 ${mode === "GRADES" ? "text-text" : "text-text/50"}`}
                        >
                            <GraduationCap className="w-4 h-4" /> Успеваемость
                        </button>
                        <button
                            onClick={() => setMode("ATTENDANCE")}
                            className={`flex-1 md:w-36 py-2 rounded-full relative z-10 text-sm font-semibold transition-colors duration-300 flex items-center justify-center gap-2 ${mode === "ATTENDANCE" ? "text-text" : "text-text/50"}`}
                        >
                            <Users className="w-4 h-4" /> Посещаемость
                        </button>
                    </div>
                </GlassCard>
            </div>
                ...
                <div className="overflow-auto flex-1 custom-scrollbar">
                    <table className="w-full text-left border-collapse min-w-[1000px]">
                        <thead className="sticky top-0 z-20">
                            <tr>
                                <th className="p-4 bg-background/10 backdrop-blur-xl border-b border-white/10 text-text min-w-[${FIRST_COL_WIDTH}px] sticky left-0 z-30 shadow-[4px_0_12px_rgba(0,0,0,0.1)]">
                                    Студент
                                </th>
                                {currentDays.map((day, i) => (
                                    <th
                                        key={i}
                                        className="p-3 bg-white/5 backdrop-blur-lg border-b border-white/10 text-center text-text/70 text-xs font-normal min-w-[50px]"
                                    >
                        ...
                        <tbody className="divide-y divide-white/5">
                            {MOCK_STUDENTS.map((student) => (
                                ...
                                >
                                    <td className="p-4 font-medium text-text bg-background/10 backdrop-blur-md sticky left-0 z-20 border-r border-background/10 shadow-[4px_0_12px_rgba(0,0,0,0.1)]">
                                        {student.name}
                                    </td>
                                    {currentDays.map((_, relativeIndex) => {
                                        ...
                                        if (mode === "GRADES") {
                                            ...
                                            if (present === true) {
                                                cellContent = (
                                                    <span className="text-text/10 text-xl">
                                                        ·
                                                    </span>
                                                );
                    ...
                    <div className="flex items-center justify-between ml-[${FIRST_COL_WIDTH}px] p-3 border-b border-white/10 bg-white/10 backdrop-blur-md z-30">
                        <button
